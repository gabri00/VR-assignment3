...
def check_gps(event):
	...
	rospy.loginfo("Current GPS data: Latitude: %f, Longitude: %f, Altitude: %f" % (gps_data.gnss.geo_point.latitude, gps_data.gnss.geo_point.longitude, gps_data.gnss.geo_point.altitude))
	...
	if is_in_area.any():
		...
		area_limit = 120
		rospy.loginfo("Drone is not within a flight restriction area")

	elevation_cli = rospy.ServiceProxy("elevation_srv", Elevation_srv)
	elevation_req = Elevation_srvRequest()
	elevation_req.latitude = float(gps_data.gnss.geo_point.latitude)
	elevation_req.longitude = float(gps_data.gnss.geo_point.longitude)
	resp = Elevation_srvResponse()
	try:
		resp = elevation_cli.call(elevation_req)
...
def main():
	...
	rospy.Timer(rospy.Duration(3), check_gps)
